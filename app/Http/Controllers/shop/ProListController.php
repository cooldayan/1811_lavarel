...
class ProListController extends Common
{
    ...
    public function proInfo($goods_id)
    {
       ...
       if(!$datas){
            ...
            cache(['goods_'. $goods_id=>$datas],12*60);
        ...
        $ress=PlModel::orderby('create_time','desc')->paginate(2);
        if(request()->ajax()){
            return view('shop.prolist.ajax',['data'=>$data,'goodsInfo'=>$goodsInfo,'res'=>$res,'datas'=>$datas,'ress'=>$ress,'goods_id'=>$goods_id]);
        }
        return view('shop.prolist.proInfo',['data'=>$data,'goodsInfo'=>$goodsInfo,'res'=>$res,'datas'=>$datas,'ress'=>$ress,'goods_id'=>$goods_id]);
    ...
    public function ajaxAddBuys(){
        $data=\request()->all();
        $res=PlModel::create($data);
        if($res){
